...
let decodeIntWrapped json =
  match Aeson.Decode.
    { iw = field "iw" (fun a -> unwrapResult ((decodeWrapper (wrapResult Aeson.Decode.int)) a)) json
    }
  with
  | v -> Js_result.Ok v
...
let decodeComplexWrapped json =
  match Aeson.Decode.
    { cw = field "cw" (fun a -> unwrapResult (decodeWrapper (wrapResult (Aeson.Decode.either (Aeson.Decode.optional Aeson.Decode.string) Aeson.Decode.float)) a)) json
    }
  with
  | v -> Js_result.Ok v
